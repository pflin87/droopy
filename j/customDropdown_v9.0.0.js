...
var DD = function(obj, sts, key) {
    ...
    if (!DD.lastRef['iseventBindOnDocument']) {
        $(document).on('click.dd', function(e) {
            if (!$(e.target).parents('.ddwn').length) {
                var dpLayer = $('.ddwn');
    ...
    X.replaceTag = function(tag) {
        return (tag.indexOf('<') != -1) ? breakStringForBold(tag) : tag;
    }
    ...
    function showDD() { // show dropdown 
        ...
        /*//start dropdown from the beginning (top)
        if (X.chkBox) { //Multi Select Dropdown Case
            X.dpLyr.find(".content").animate({
                scrollTop: '0'
            }, 500);
        } else {
            if (X.inpHid.val() == "") { //Single Select Dropdown Case if no value is selected
                X.dpLyr.find(".content").animate({
                    scrollTop: '0'
                }, 500);
            }
